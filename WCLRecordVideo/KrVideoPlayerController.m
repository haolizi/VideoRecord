...
- (void)dismissWithType:(BOOL)isSave
{
    ...
    [UIView animateWithDuration:kVideoPlayerControllerAnimationTimeinterval animations:^{
        ...
        if (isSave) {
            if (self.dimissCompleteBlock) {
                self.dimissCompleteBlock(YES);
            }
        }
        //返回重录
        else {
            if (self.dimissCompleteBlock) {
                self.dimissCompleteBlock(NO);
            }
        }
    }];
...
{
    [self dismissWithType:NO];
}

- (void)closeButtonClick
{
    [self dismissWithType:YES];
}
...
- (void)setDeviceOrientationLandscapeRight{
    ...
    CGRect frame = CGRectMake((SCREEN_HEIGHT - SCREEN_WIDTH) / 2, (SCREEN_WIDTH - SCREEN_HEIGHT) / 2, SCREEN_WIDTH, SCREEN_HEIGHT);;
