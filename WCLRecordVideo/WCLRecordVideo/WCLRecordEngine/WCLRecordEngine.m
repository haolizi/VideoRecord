...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.maxRecordTime = 60;//默认最大录制时间为一分钟
    }
    return self;
}
...
- (NSInteger)cx
{
    return self.recordOrientation == RecordOrientationPortrait? 720 : 1280;
}

- (NSInteger)cy
{
    return self.recordOrientation == RecordOrientationPortrait? 1280 : 720;
}
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.recordEncoder = [WCLRecordEncoder encoderForPath:self.videoPath Height:self.cy width:self.cx channels:_channels samples:_samplerate];
        }
        //判断是否中断录制过
        if (self.discont) {
            if (isVideo) {
